...
or updates drafts on WeChat Official Account, Zhihu, Juejin, CSDN, OSChina, and
BlogsCN — all from a single Markdown source note. It saves drafts only; it does not
publish.
...
- Zhihu, Juejin, CSDN, OSChina, and BlogsCN only operate on the user's
  already-logged-in visible editor — no private APIs, no cookie reading.
...
packages/
  core/              Markdown AST, Obsidian embeds, MathJax, image dedup, themes & CSS inlining
...
WeChat Official Account requires only the Obsidian plugin. Zhihu, Juejin, CSDN,
OSChina, and BlogsCN also need the Chromium extension ZIP from the same Release;
unzip it and load it as an unpacked extension in Chrome or Edge.
...
`http://127.0.0.1/*` (to read one-shot local resources); the five browser-platform
...
publishing is always manual. CSDN, OSChina, and BlogsCN have passed offline DOM
fixture, image-paste state-machine, and build checks but have not yet claimed real
...
boundary. The historical writing entry-point for SegmentFault returned HTTP 410 in
an anonymous probe on 2026-07-31, so it was not included this time.
...
源稿创建或更新微信公众号、知乎、掘金、CSDN、开源中国和博客园草稿。它只保存草稿，
不执行最终发布。
...
资源）；五个浏览器平台的域名都是按需申请的可选权限，不申请 `cookies`。CI 会审计
...
开源中国和博客园目前已通过离线 DOM fixture、图片粘贴状态机和构建检查，尚未宣称完成
真实账号验收。
...
零 Cookie/零私有接口边界。SegmentFault 的历史写作入口在 2026-07-31 的匿名探测中
返回 HTTP 410，因此本次未加入。
